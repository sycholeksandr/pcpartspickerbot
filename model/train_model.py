...
from model.pcbuild_model import PCBuildModel  # або ./pcbuild_model, якщо запускаєш з локального каталогу
...
def main():
    ...
    df = pd.read_csv("../data/final_cleaned_builds.csv")

    categorical_columns = ["CPU", "Motherboard", "Memory", "Video Card", "Power Supply"]
    numeric_columns = ["Total Price", "Gaming Score", "Work Score"]
    ...
    torch.save(model.state_dict(), "pcbuild_model.pt")
